...
// Semi-transparent walls rising along the observation-area boundary give the
...
function buildAreaWalls(polygons, baseY, wallHeight) {
  const positions = [];
  const pushRing = ring => {
  ...
  mesh.userData.kind = 'observation-wall';
  mesh.renderOrder = 2;
  return mesh;
}

export function createObservationOverlay({ rects, baseY, lit = false, draft = false, wallHeight = 0 }) {
  ...
  if (wallHeight > 0 && polygons.length > 0) {
    group.add(buildAreaWalls(polygons, baseY, wallHeight));
  }
  return group;
}
