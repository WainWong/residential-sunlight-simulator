...
- 聚焦时**隐藏所有建筑实体 mesh**（选中建筑的整栋 + 所有邻楼），
  只留一个干净的当前层视图，符合「只关心这一层」。
- 改画：本层高度的**墙体轮廓线**（由 `createFootprint` 外环生成 `LineLoop`，
  置于当前楼层顶/底高度）+ 已有地板网格 slab + 选区 overlay，提供方位与房间轮廓感。
- **遮挡关系不靠肉眼看邻楼**，而由采光结果（被照亮的格子高亮）反馈——那是准确的信息源。
  故聚焦编辑期间邻楼也隐藏；退出聚焦后恢复全部实体可见。
  > `floorVisibility` 语义简化：聚焦时对所有建筑返回 false（全隐藏），退出恢复。
- 退出聚焦：恢复所有建筑实体 `visible = true`，移除轮廓线。
