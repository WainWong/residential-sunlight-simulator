...
- **画房间预览 (Draw Preview)** — 画房间/擦除时的实时反馈。进入画房间即把[[室内取景态]]**当前层以上的楼体临时掀开**(俯视这层),拖动时在**当前层顶面(bandTop)贴一个 2D 半透明矩形**跟手——贴朝上的顶面,斜俯永不被挡,位置真实(所见即所得),不与立体楼体抢深度、不用穿透特效。**松开鼠标即时落地**:该块立刻并入草稿并跑一次 CSG 挖空(变半透明空腔)+ 触发采光计算,但**房间未结束**,可继续拖下一块拼进同一房间(L/凹形);块之间**允许暂时不连通**,直到点"完成房间"才统一校验连通性并收尾命名。区别于旧的"悬浮小矩形预览"(会被楼体挡、位置感假)与"每帧 CSG"(卡)。
