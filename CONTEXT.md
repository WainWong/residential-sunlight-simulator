...
- **室内视图 (Interior View)** — 用户进入某个房间内部、观察真实太阳光斑分布的视图模式。由 `interiorRoomId` 触发。完整生命周期：进入（飞相机、按 [[天花显隐]] 档应用盖子、聚焦阴影相机）、维持（天花档变化即重应用、被挡的外墙每帧淡化）、退出（盖子还原、墙恢复、阴影复位）。盖子显隐是手动的([[天花显隐]]),不再随相机高度自动揭盖。归属于 `src/scene/createInteriorView.js`。

- **盖子 (Lid)** — 观察层顶面及以上被整块隐藏/半透明的几何:顶层房间的独立顶盖 mesh，或非顶层时上方的整段楼层。由 [[天花显隐]] 的三档手动控制,编辑房间与查看采光共用。

- **天花显隐 (Ceiling Display)** — 用户对"盖子"(见 [[盖子]])的显式三档控制 `view.ceiling`：`show`(完整盖着) / `ghost`(半透明) / `hide`(掀开，默认)。**全局单一状态**,编辑房间与查看采光两视图**共用同一档**、共用同一底层原语 `applyCeiling`(`src/scene/ceilingVisibility.js`)。**纯视觉,不影响采光计算**——采光始终把天花/地板/屋顶当障碍物(见 [[开口的采光语义]])。查看采光**不再**按相机高度自动揭盖(旧的第一人称自动掀盖已移除);挡视线的**外墙**仍每帧动态淡化(侧墙遮挡,与天花是两回事)。
...
- **操作模型 (Pointer Model)** — 全局统一,不随视图/工具变:**左键=仅选择或绘制**(选实体、或画/擦房间;由场景自己的指针监听处理,OrbitControls 不占用左键)、**右键=旋转**、**中键=平移**、**Shift+左键=平移**(触摸板/无中键后备)、**滚轮=缩放**。触摸端:单指旋转、双指缩放/平移。右键旋转需在 canvas 屏蔽浏览器右键菜单。归属 `createCameraRig`;旋转常开(右键),不再按工具"锁左键旋转",`setEditControls` 退化为确保控件启用。旋转是高频操作,故放最好按的右键。
...
- **画房间预览 (Draw Preview)** — 画房间/擦除时的实时反馈。进入画房间即把[[室内取景态]]**当前层以上的楼体临时掀开**(有效天花档 `effectiveCeiling`:画/擦工具激活时强制 hide),拖动时在**当前层顶面(bandTop)贴一个 2D 半透明矩形**跟手——**拾取平面也在 bandTop**(否则视觉在顶、拾取在底会有视差偏移、不跟手)。贴朝上的顶面斜俯永不被挡,位置真实,不与立体楼体抢深度、不用穿透特效。**松开鼠标即时落地**:该块并入草稿(`createAppendRoomRectCommand`),已画块持续显示为半透明累积(create/edit 一致)。绘制期**允许暂时不连通**(append 用 `validateRoomRects(..., {allowDisconnected:true})`,只挡重叠/越界/占用他室),连通性**留到"完成房间"** 统一校验(`createFinishRoomCommand` 拒绝不连通并提示);编辑已有房间时新块并入当前房间。区别于旧的"悬浮小矩形预览"(会被楼体挡、位置感假)与"每帧 CSG"(卡)。
