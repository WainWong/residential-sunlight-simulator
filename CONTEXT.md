...
- **画房间预览 (Draw Preview)** — 画房间/擦除时的实时反馈。进入画房间即把[[室内取景态]]**当前层以上的楼体临时掀开**(有效天花档 `effectiveCeiling`:画/擦工具激活时强制 hide),拖动时在**当前层顶面(bandTop)贴一个 2D 矩形**跟手(拾取平面也在 bandTop,避免视差偏移)。**草稿 = 一个未提交的房间**,和已完成房间**走同一条 CSG 挖空管线**:楼体重建签名 = `revision : highlighted : 草稿指纹`,草稿的 rects 一变(**松手/append**,离散事件,不卡),楼体就把草稿那块**真的挖空**(墙/洞成型,和"完成房间"后一致)——不是贴片、不整栋变半透明、也不搞每段材质。草稿"正在画"的标记 = 在草稿区域**楼板底面贴一张半透明蓝色平面**,完成后去掉。**拖动过程中**用便宜的蓝色平面跟手占位(不每帧 CSG),**松手才真挖**。合并判定:append 对**本房间自己的块取并集**(`applyRectEdit 'draw'`,重叠也能合并),只挡与**别的房间**重叠/越界;连通性留到"完成房间"统一校验。取代旧的"悬浮 overlay 贴片预览"(位置假、会飘)。
